...
import { Database, ChevronLeft, Plus, X, Upload, RefreshCw, Download, FileText } from 'lucide-react'
...
export default function DataIntelligence() {
  ...
  // ── Export: Download Markdown summary ────────────────────────────────────
  function downloadSummary() {
    const pattern = recommendation?.primaryPattern
    const today   = new Date().toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' })
    const scoreLabel = s => s <= 2 ? 'Healthy' : s <= 3 ? 'At risk' : 'Critical'

    const lines = [
      `# Data Intelligence Assessment — ${clientName}`,
      `**Date:** ${today}`,
      pattern ? `**Recommended Solution:** ${pattern.name}` : '',
      '',
      '---',
      '',
      '## Executive Summary',
    ...
    if (healthProfile) {
      ...
      const dims = [
        { key: 'siloedData',      label: 'Data Silos' },
      ...
      dims.forEach(d => {
        const score = healthProfile[d.key]
        if (score != null) lines.push(`- **${d.label}:** ${score}/5 — ${scoreLabel(score)}`)
      })
      lines.push('')
    }

    const activeFn = findings.filter(f => !dismissedFindings.includes(f.finding))
    if (activeFn.length) {
      lines.push('## Key Findings')
      activeFn.forEach(f => lines.push(`- **[${f.severity || 'Finding'}]** ${f.finding}`))
      lines.push('')
    }
    ...
    if (pattern) {
      ...
      if (pattern.rationale)      lines.push(`**Rationale:** ${pattern.rationale}`)
      if (pattern.complexity)     lines.push(`**Complexity:** ${pattern.complexity}`)
      if (pattern.solves?.length) {
        lines.push('')
        lines.push('**Solves:**')
        pattern.solves.forEach(s => lines.push(`- ${s}`))
      }
      if (pattern.doesNotSolve?.length) {
        lines.push('')
        lines.push('**Does Not Solve:**')
        pattern.doesNotSolve.forEach(s => lines.push(`- ${s}`))
      }
      if (pattern.keyProducts?.length) {
        lines.push('')
        lines.push('**Recommended Products:**')
        pattern.keyProducts.forEach(p => lines.push(`- ${p.name}${p.alreadyHave ? ' *(already have)*' : ''}`))
      }
      lines.push('')
    }

    if (recommendation?.zonesEngagement) {
      const eng = recommendation.zonesEngagement
      lines.push('## Zones Engagement Model')
      lines.push(`**Type:** ${eng.type || ''}`)
      if (eng.investmentRange) lines.push(`**Estimated Investment:** ${eng.investmentRange}`)
      if (eng.phases?.length) {
        lines.push('')
        lines.push('**Phases:**')
        eng.phases.forEach(p => lines.push(`- **${p.name}** (${p.duration}): ${p.deliverable}`))
      }
      lines.push('')
    }

    if (sources.length) {
      lines.push('## Data Sources Inventoried')
      sources.forEach(s => {
        lines.push(`- **${s.name}** (${s.category || 'Unknown category'})${s.volume ? ` — ${s.volume}` : ''}${s.frequency ? `, ${s.frequency}` : ''}`)
      })
      lines.push('')
    }

    lines.push('---')
    lines.push('_Generated by Zones Compass · Data Intelligence_')

    const blob = new Blob([lines.join('\n')], { type: 'text/markdown' })
    const url  = URL.createObjectURL(blob)
    const a    = document.createElement('a')
    a.href     = url
    a.download = `${clientName.replace(/\s+/g, '-')}-Data-Intelligence-Summary-${new Date().toISOString().split('T')[0]}.md`
    a.click()
  ...
  async function downloadPDF() {
    try {
      ...
      function txt(t, x, yy, w, size = 9, color = C.black, style = 'normal') {
        ...
        const lines = pdf.splitTextToSize(clean(t), w)
        pdf.text(lines, x, yy)
      ...
      function hdr() {
        ...
        pdf.text('ZONES COMPASS · DATA INTELLIGENCE', mg, 6.5)
        pdf.setFont('helvetica', 'normal'); pdf.setFontSize(8); sc(C.white)
        pdf.text(`${clientName} — Data Intelligence Report`, mg, 13)
        pdf.text(new Date().toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' }), pageW - mg, 13, { align: 'right' })
        return 26
      }
      function ftr(total) {
        sf(C.navy); pdf.rect(0, pageH - 9, pageW, 9, 'F')
        pdf.setFontSize(6.5); pdf.setFont('helvetica', 'normal'); sc([160, 180, 220])
        pdf.text('Confidential — Zones Compass · Data Intelligence', mg, pageH - 3)
        pdf.text(`${pageNum} / ${total}`, pageW - mg, pageH - 3, { align: 'right' })
      }
      function np() {
        pageNum++; pdf.addPage(); y = hdr()
      }
      function cy(need = 14) { if (y + need > pageH - 14) np() }
      function secHdr(title) {
        cy(12); sf(C.navy); pdf.rect(mg, y, cW, 8, 'F')
        pdf.setFontSize(8.5); pdf.setFont('helvetica', 'bold'); sc(C.white)
        pdf.text(title, mg + 4, y + 5.5); y += 11
      }

      // PAGE 1 — Cover
      y = hdr()
      pdf.setFontSize(18); pdf.setFont('helvetica', 'bold'); sc(C.navy)
      pdf.text(clean(clientName), mg, y + 8); y += 12
      pdf.setFontSize(10); pdf.setFont('helvetica', 'normal'); sc(C.blue)
      pdf.text('Data Intelligence Assessment Report', mg, y); y += 6
      sd(C.blue); pdf.setLineWidth(0.4); pdf.line(mg, y, pageW - mg, y); y += 10

      const pattern = recommendation?.primaryPattern
      if (pattern?.name) {
        pdf.setFontSize(11); pdf.setFont('helvetica', 'bold'); sc(C.navy)
        pdf.text(`Recommended: ${clean(pattern.name)}`, mg, y); y += 8
      }

      secHdr('EXECUTIVE SUMMARY')
      const sh = txt(summary || 'No summary available.', mg + 4, y, cW - 8); y += sh + 10

      // Health profile scores
      if (healthProfile) {
        secHdr('DATA HEALTH PROFILE')
        const scoreColor = s => s <= 2 ? C.green : s <= 3 ? C.amber : C.red
        const scoreLabel = s => s <= 2 ? 'Healthy' : s <= 3 ? 'At risk' : 'Critical'
        const dims = [
          { key: 'siloedData',      label: 'Data Silos' },
          { key: 'dataquality',     label: 'Data Quality' },
        ...
        dims.forEach(d => {
          ...
          cy(10)
          const barW = cW - 8
          const fill  = (score / 5) * barW
          const lc    = scoreColor(score)
          sf(C.lgray); pdf.rect(mg + 4, y, barW, 6, 'F')
          sf(lc); pdf.rect(mg + 4, y, fill, 6, 'F')
          pdf.setFontSize(7.5); pdf.setFont('helvetica', 'bold'); sc(C.black)
          pdf.text(d.label, mg + 4, y - 1)
          pdf.setFont('helvetica', 'normal'); sc(lc)
          pdf.text(`${score}/5 — ${scoreLabel(score)}`, pageW - mg - 4, y - 1, { align: 'right' })
          y += 10
        })
        y += 4
      }
      ...
      np(); secHdr('KEY FINDINGS')
      const activeFn = findings.filter(f => !dismissedFindings.includes(f.finding))
      if (activeFn.length === 0) {
        txt('No findings recorded.', mg + 4, y, cW - 8, 9, C.muted); y += 10
      } else {
        activeFn.forEach(f => {
          cy(12)
          const sev = f.severity || 'Finding'
          const sevC = sev === 'Critical' ? C.red : sev === 'Warning' ? C.amber : C.blue
          sf(C.lgray); sd(C.bgray); pdf.setLineWidth(0.2)
          pdf.roundedRect(mg, y, cW, 10, 1.5, 1.5, 'FD')
          sf(sevC); pdf.rect(mg, y, 2.5, 10, 'F')
          pdf.setFontSize(7); pdf.setFont('helvetica', 'bold'); sc(sevC)
          pdf.text(sev.toUpperCase(), mg + 5, y + 4)
          const h = txt(f.finding, mg + 5 + pdf.getStringUnitWidth(sev.toUpperCase()) * 7 / pdf.internal.scaleFactor + 4, y + 4, cW - 20, 8)
          y += Math.max(10, h + 6) + 3
        })
      }
      y += 4
      ...
      if (pattern) {
        secHdr('RECOMMENDATION')
        txt(`${clean(pattern.name)} — ${clean(pattern.rationale || '')}`, mg + 4, y, cW - 8, 9); y += 10

        if (pattern.solves?.length) {
          pdf.setFontSize(8); pdf.setFont('helvetica', 'bold'); sc(C.green)
          pdf.text('SOLVES', mg + 4, y); y += 5
          pattern.solves.forEach(s => { cy(6); txt(`• ${s}`, mg + 6, y, cW - 10, 8); y += 5.5 })
          y += 3
        }
        if (pattern.doesNotSolve?.length) {
          pdf.setFontSize(8); pdf.setFont('helvetica', 'bold'); sc(C.red)
          pdf.text('DOES NOT SOLVE', mg + 4, y); y += 5
          pattern.doesNotSolve.forEach(s => { cy(6); txt(`• ${s}`, mg + 6, y, cW - 10, 8); y += 5.5 })
          y += 3
        }
        if (pattern.keyProducts?.length) {
          pdf.setFontSize(8); pdf.setFont('helvetica', 'bold'); sc(C.navy)
          pdf.text('RECOMMENDED PRODUCTS', mg + 4, y); y += 5
          pattern.keyProducts.forEach(p => {
            cy(6)
            const have = p.alreadyHave ? ' (already have)' : ''
            txt(`• ${clean(p.name)}${have}`, mg + 6, y, cW - 10, 8); y += 5.5
          })
          y += 3
        }
      }
      ...
      if (recommendation?.zonesEngagement) {
        const eng = recommendation.zonesEngagement
        np(); secHdr('ZONES ENGAGEMENT MODEL')
        txt(clean(eng.type || ''), mg + 4, y, cW - 8, 10, C.navy, 'bold'); y += 8
        if (eng.investmentRange) { txt(`Estimated Investment: ${clean(eng.investmentRange)}`, mg + 4, y, cW - 8, 9); y += 8 }
        eng.phases?.forEach((p, i) => {
          cy(14)
          sf(i % 2 === 0 ? C.lgray : C.white); pdf.rect(mg, y, cW, 12, 'F')
          pdf.setFontSize(8.5); pdf.setFont('helvetica', 'bold'); sc(C.navy)
          pdf.text(clean(p.name), mg + 4, y + 5)
          pdf.setFont('helvetica', 'normal'); sc(C.muted)
          pdf.text(clean(p.duration || ''), pageW - mg - 4, y + 5, { align: 'right' })
          txt(clean(p.deliverable || ''), mg + 4, y + 9, cW - 8, 7.5, C.muted)
          y += 15
        })
        y += 5
      }
      ...
      // Data sources
      if (sources.length) {
        np(); secHdr('DATA SOURCES INVENTORIED')
        sources.forEach((s, i) => {
          cy(10)
          sf(i % 2 === 0 ? C.lgray : C.white); pdf.rect(mg, y, cW, 8, 'F')
          pdf.setFontSize(8); pdf.setFont('helvetica', 'bold'); sc(C.navy)
          pdf.text(clean(s.name), mg + 4, y + 5)
          sc(C.muted); pdf.setFont('helvetica', 'normal')
          const meta = [s.category, s.volume, s.frequency].filter(Boolean).join(' · ')
          pdf.text(clean(meta), pageW - mg - 4, y + 5, { align: 'right' })
          y += 10
        })
      }
      ...
      for (let i = 1; i <= total; i++) { pdf.setPage(i); pageNum = i; ftr(total) }

      pdf.save(`${clientName.replace(/\s+/g, '_')}-Data-Intelligence-Report-${new Date().toISOString().split('T')[0]}.pdf`)
    } catch (err) {
      ...
      <div className="di-blueprint">
        <div className="di-blueprint-header">
          ...
          <div className="di-bp-actions">
            ...
            <button className="di-btn-export" onClick={downloadSummary}>
              <FileText size={13} /> Download Summary
            </button>
            <button className="di-btn-export" onClick={downloadPDF}>
